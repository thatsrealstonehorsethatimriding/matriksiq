...
class MatriksIQ:
    ...
    def ListAccounts(self):
        payLoad = {"ApiCommands": CMD_ListAccounts}
    ...
    def SendKeepAlive(self):
        payLoad = {"KeepAliveDate": self.now, "ApiCommands": CMD_SendKeepAlive}
        return self.send(payLoad)
    ...
    now = property(
        lambda self: f"{datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%f%z')}"
    )
    new_order_id = property(
        lambda self: f"{datetime.datetime.now().strftime('%Y%m%d%H%M%S%f%z')}"
    )
...
def index():
    ...
    return render_template_string(
        ...
        <html>
            <head>
                <title>Market Sell Response</title>
            </head>
            <body>
                <h1>Market Sell Response</h1>
                <pre>{{ response }}</pre>
            </body>
        </html>
    """,
